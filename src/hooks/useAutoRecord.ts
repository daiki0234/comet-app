...
import { collection, query, where, getDocs, addDoc, updateDoc, serverTimestamp, doc, getDoc, orderBy, limit } from 'firebase/firestore'; // ★ updateDoc を追加
...
export const useAutoRecord = () => {
  ...
  const createRecord = async (data: AutoRecordInput) => {
    ...
    try {
      ...
      if (!snapshot.empty) {
        // ★★★ 追加: すでに日誌が存在する場合は、時間と延長加算だけを上書き更新する ★★★
        ...
        if (data.status === '欠席') {
          updateData.absenceAddon = 'Ⅰ';
          updateData.startTime = '';
          updateData.endTime = '';
          updateData.duration = '';
          updateData.extensionDuration = '';
          updateData.timeClass = '';
          updateData.extendedSupportAddon = '加算しない';
        } else {
          ...
          if (data.extensionMinutes && data.extensionMinutes > 0) {
            ...
            updateData.extensionDuration = String(extHours);
            if (extHours >= 2.0) updateData.extendedSupportAddon = '3（2時間以上）';
            else if (extHours >= 1.0) updateData.extendedSupportAddon = '2（1時間以上2時間未満）';
            else if (extHours >= 0.5) updateData.extendedSupportAddon = '1（30分以上1時間未満）';
          }
        }

        await updateDoc(doc(db, 'supportRecords', existingDocId), updateData);
        console.log(`[Update] ${data.userName} の既存支援日誌の時間を上書き更新しました。`);
        return; 
      }

      // ==========================================
      // 以下、日誌がまだ存在しない場合の「新規作成」処理
      // ==========================================
