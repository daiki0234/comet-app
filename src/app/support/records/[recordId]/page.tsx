...
export default function EditRecordPage({ params }: { params: { recordId: string } }) {
  ...
  const [lockInfo, setLockInfo] = useState<{ staffName: string; updatedAt: any } | null>(null);
  ...
  const [formData, setFormData] = useState({
    date: '', userId: '', userName: '', status: '' as any,
    startTime: '', endTime: '', duration: '', extensionDuration: '', 
    condition: '良好' as any, timeClass: '', extendedSupportAddon: '',
    absenceAddon: '', childcareSupport: '', individualSupport: '',
    specializedSupport: '', agencyCooperation: '', familySupport: '',
    transportation: '', independenceSupport: '', interAgencyCooperation: '',
    medicalSupport: '', selfRelianceSupport: '', intenseBehaviorSupport: '',
    welfareSpecialist: '', staffAddon: '', specializedSystem: '',
    planMissing: '', managerMissing: '', staffMissing: '',
    trainingContent: '', supportContent: '', staffSharing: '',
  });
    ...
    const initPage = async () => {
      try {
        ...
        if (rData.targetComments) {
            ...
            rData.targetComments.forEach((item: any) => { commentsObj[item.targetId] = item.comment; });
            setTargetComments(commentsObj);
        }

        if (rData.userId) {
            const q = query(collection(db, 'supportPlans'), where('userId', '==', rData.userId), where('status', '==', '本番'), orderBy('createdAt', 'desc'), limit(1));
  ...
  useEffect(() => {
    ...
    if (formData.status === '休校日') {
      ...
      if (schedule?.duration) determinedDuration = schedule.duration;
    }
    if (!determinedDuration) {
      determinedDuration = formData.status === '放課後' ? '2.0' : '3.5';
    }
    let newClass = '';
  ...
  // --- 自動算出ロジック (延長支援) ---
  useEffect(() => {
    if (loading || formData.status === '欠席') return;
    let extAddon = '加算しない';
    ...
    setFormData(prev => prev.extendedSupportAddon === extAddon ? prev : { ...prev, extendedSupportAddon: extAddon });
  ...
  const handleSelectUser = async (user: UserData) => {
    ...
    const q = query(collection(db, 'supportPlans'), where('userId', '==', user.id), where('status', '==', '本番'), orderBy('createdAt', 'desc'), limit(1));
    const snap = await getDocs(q);
    setActivePlan(!snap.empty ? snap.docs[0].data() : null);
    setTargetComments({});
  };
      ...
      {isReadOnly && lockInfo && (
        <div className="mb-6 bg-red-100 border-l-4 border-red-500 p-4 rounded-r shadow-md animate-pulse">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-red-800 font-bold">現在、<span className="underline">{lockInfo.staffName}</span> さんがこの記録を編集しています。</p>
              <p className="text-red-700 text-xs mt-1">データの衝突を防ぐため、現在は読み取り専用モードになっています。</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 h-full pb-20 ${isReadOnly ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="relative">
                ...
                <input type="text" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setShowSuggestions(true); }} onFocus={() => setShowSuggestions(true)} className="w-full border p-2 rounded" placeholder="検索..." />
                {showSuggestions && (
        ...
        <div className="space-y-6 overflow-y-auto pr-2">
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 space-y-4">
            ...
            {!activePlan ? <p className="text-sm text-gray-400 p-4 text-center">本番の個別支援計画が存在しません</p> : (
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="text-xs font-bold text-gray-500 block mb-1">長期目標</label><div className="bg-gray-50 p-2 rounded text-sm border whitespace-pre-wrap">{activePlan.longTermGoal}</div></div>
                  <div><label className="text-xs font-bold text-gray-500 block mb-1">短期目標</label><div className="bg-gray-50 p-2 rounded text-sm border whitespace-pre-wrap">{activePlan.shortTermGoal}</div></div>
                </div>
                {activePlan.supportTargets?.sort((a:any,b:any)=>Number(a.displayOrder)-Number(b.displayOrder)).map((target: any, idx: number) => (
                  <div key={idx} className="border-t pt-4">
                    <div className="mb-2"><span className="bg-blue-100 text-blue-800 text-xs font-bold px-2 py-1 rounded">支援目標 {target.displayOrder}</span></div>
                    ...
                    <div>
                      ...
                      <textarea className="w-full border p-2 rounded text-sm h-20 resize-y" placeholder="コメントを入力..." value={targetComments[target.id] || ''} onChange={(e) => setTargetComments({...targetComments, [target.id]: e.target.value})} />
                    </div>
                  </div>
                ))}
              </div>
...
const SelectField = ({ label, value, onChange, options }: { label: string, value: string, onChange: (v: string) => void, options: string[] }) => (
  <div className="flex flex-col">
    ...
    <select value={value} onChange={(e) => onChange(e.target.value)} className="border p-2 rounded text-sm bg-white">{options.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select>
  </div>
);