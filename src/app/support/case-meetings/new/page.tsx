...
export default function NewCaseMeetingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'caseMeetings'), {
        date,
  ...
  return (
    <AppLayout pageTitle="ケース担当者会議 新規作成">
                      ...
                      <div className="relative">
                        ...
                        {activeSearchRow === index && (
                          <div className="absolute z-10 w-full bg-white border shadow-lg max-h-40 overflow-y-auto mt-1 rounded">
                            {users.filter(u => `${u.lastName} ${u.firstName}`.includes(row.userName)).map(u => (
                              <div 
                                key={u.id} 
