...
export default function NewPlanPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user && user.displayName) {
        setBasicInfo(prev => {
          if (!prev.author) {
            return { ...prev, author: user.displayName || '' };
          }
          return prev;
        });
      }
  ...
  const handleSave = async () => {
    ...
    try {
        await addDoc(collection(db, 'supportPlans'), {
            ...basicInfo,
            schedules,
            remarks,
            supportTargets,
        ...
        console.error(e);
        toast.error("保存失敗", { id: toastId });
    }
  };

  return (
    <AppLayout pageTitle="個別支援計画 作成">
